...
## Test Case 01: Compose Login through CAM Admin
  ...
  Scenario: Unsuccessful Login using invalid username valid password
    Given Open the browser and launch the application
    When Enter a invalid email 'test@globalwavenet.com' and valid password 'Test@123'
    And Click SignIn
    Then Verify Error Message

  Scenario: Unsuccessful Login using valid username invalid password
    Given Open the browser and launch the application
    When Enter a valid email 'super_admin@globalwavenet.com' and invalid password 'Test'
    And Click SignIn
    Then Verification of Error Message

  Scenario: Unsuccessful Login using invalid username invalid password
    Given Open the browser and launch the application
    When Enter a invalid email 'test@gmail.com' and invalid password 'Test'
    And Click SignIn
    Then Verify Error Message
  ...
  Scenario: Empty password with username
    Given Open the browser and launch the application
    When Enter username 'super_admin@globalwavenet.com' and empty password
    And Click SignIn
    Then Verify Error Popup
